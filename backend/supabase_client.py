...
def save_analysis_to_supabase(data: dict):
    ...
    existing = supabase.table(settings.SUPABASE_ANALYSIS_TABLE).select("*").eq("task_id", task_id).execute().data
    ...
    if existing and len(existing) > 0:
        ...
        response = supabase.table(settings.SUPABASE_ANALYSIS_TABLE).upsert(data, on_conflict=['task_id']).execute()
    else:
